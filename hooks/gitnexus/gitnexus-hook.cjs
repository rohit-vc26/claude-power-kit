 ...
 *
 * SessionStart — checks index freshness on boot; reports status + flags stale
...
function resolveCliPath() {
  // Try common install locations (macOS Homebrew, Linux npm global, nvm, local)
  const candidates = [
...
/**
 * SessionStart handler — check index freshness on boot.
 *
 * Reads .gitnexus/meta.json, compares lastCommit vs git HEAD,
 * and prints a status line so Claude knows the graph state before work begins.
 * Does NOT auto-run analyze (blocks too long); instead flags stale clearly.
 */
function handleSessionStart(input) {
  ...
  if (!gitNexusDir) return; // no index — not a gitnexus project, stay silent

  let meta = {};
  try {
    meta = JSON.parse(fs.readFileSync(path.join(gitNexusDir, 'meta.json'), 'utf-8'));
  } catch {
    /* no meta.json — index may be empty */
  }

  const lastCommit = meta.lastCommit || '';
  const stats = meta.stats || {};
  const symbols = stats.symbols || 0;
  const relationships = stats.relationships || 0;
  const embeddings = stats.embeddings || 0;
  const hadEmbeddings = embeddings > 0;
  ...
  const isStale = currentHead && lastCommit && currentHead !== lastCommit;
  const neverIndexed = !lastCommit;
  const analyzeCmd = `npx gitnexus analyze${hadEmbeddings ? ' --embeddings' : ''}`;

  let msg;
  if (neverIndexed) {
    msg = `GitNexus: no index found for this project. Run \`${analyzeCmd}\` to build the knowledge graph.`;
  } else if (isStale) {
    msg = `GitNexus: index is STALE (indexed: ${lastCommit.slice(0, 7)}, HEAD: ${currentHead.slice(0, 7)}). ` +
      `${symbols} symbols / ${relationships} relationships / ${embeddings} embeddings. ` +
      `Run \`${analyzeCmd}\` before editing.`;
  } else {
    msg = `GitNexus: index FRESH ✓ — ${symbols} symbols, ${relationships} relationships` +
      (embeddings > 0 ? `, ${embeddings} embeddings` : '') +
      ` (${lastCommit ? lastCommit.slice(0, 7) : 'unknown'})`;
  }
